...
def create_user(user: UserCreate, db: Session = Depends(get_db), current_admin: User = Depends(get_current_admin)):
    try:
        ...
        db_user = User(
            ...
            daily_pay=daily_pay,
            pay_type=user.pay_type or "hourly",
            default_shift_start=user.default_shift_start or "09:00:00",
            default_shift_end=user.default_shift_end or "18:00:00",
...
def update_user(user_id: int, user_update: UserUpdate, db: Session = Depends(get_db), current_admin: User = Depends(get_current_admin)):
    ...
    if "hourly_pay" in update_data and update_data["hourly_pay"] is not None and update_data["hourly_pay"] < 0:
        raise HTTPException(status_code=400, detail="Hourly pay cannot be negative")
    if "daily_pay" in update_data and update_data["daily_pay"] is not None and update_data["daily_pay"] < 0:
        raise HTTPException(status_code=400, detail="Daily pay cannot be negative")
